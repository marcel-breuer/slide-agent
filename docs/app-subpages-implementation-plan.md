...
| ----------------------------------------------- | ---------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `/app/projects`                                 | Import panel is implemented, but project listing is not database-backed yet. | Add user-owned project list, create project action, recent presentations, archived project entry point, and import-to-project flow.            |
| `/app/projects/[projectId]`                     | Placeholder page.                                                            | Add project detail with presentations, create/rename/archive/restore actions, project metadata, and links into briefing, editor, and export.   |
...
| `/app/presentations/[presentationId]`           | Redirects to `/app/presentations/demo-presentation/editor`.                  | Replace demo redirect with a real presentation overview that routes to briefing, storyline, editor, and export for the requested presentation. |
...
1. **Project and presentation management**
   - Build the real `/app/projects` and `/app/projects/[projectId]` pages first because they become the entry point for imported, created, and archived decks.
   - Replace hardcoded presentation routing with real presentation IDs from the database.
   - Acceptance: a user can create a project, create or import a presentation, open it in the editor, archive it, and restore it.

2. **Presentation workflow pages**
...
3. **Presentation defaults and language settings**
...
6. **Profile settings and account lifecycle**
...
The next implementation package should be project and presentation management. It unlocks the remaining contextual subpages and removes the hardcoded demo presentation redirect.
